...
export async function POST(request: NextRequest) {
  try {
    console.log('🧪 Testing Google Calendar integration (PUBLIC)...');
    ...
    const testEvent = createAppointmentEvent({
      title: 'Public Test Calendar Integration',
      date: new Date().toISOString().split('T')[0], // Today
      time: new Date(Date.now() + 60 * 60 * 1000).toISOString().split('T')[1].substring(0, 5), // 1 hour from now
      duration: 30,
      description: 'This is a public test event created by the Concierge AI system',
...
export async function GET() {
  return NextResponse.json({
    message: 'Public Google Calendar Test Endpoint',
    usage: 'POST to test calendar integration (no auth required)',
    requiredEnvVars: [
